...
| **Runtime & Architecture** | Python 3.11, `x86_64` (FastAPI + Mangum via Function URL) |
| **Region** | `us-east-1` |
...
| **Sandbox Verifier** | GitHub Actions runner via test mirror repos (`backend/app/sandbox/github_actions_runner.py`). AWS CodeBuild, GCP Cloud Build, and Local Docker runners are retired. |
| **SSM Parameter Store** | `/haunter/GITHUB_SANDBOX_APP_PRIVATE_KEY` (SecureString holding GitHub App PEM for sandbox runs; never in state, .env, or zip) |
...
├── ssm.tf             # SSM parameter store references (GitHub App private key)
...
| SSM param (GitHub App PEM) | `/haunter/GITHUB_SANDBOX_APP_PRIVATE_KEY` |
| Account ID | `452258152602` |

> **SSM Parameter Store Note**: `/haunter/GITHUB_SANDBOX_APP_PRIVATE_KEY` holds the GitHub App private key PEM (SecureString) used by the GitHub Actions sandbox runner (`backend/app/sandbox/github_actions_runner.py`). Lambda fetches it at runtime via boto3 (`WithDecryption=True`) and caches it in memory. This sensitive key is never committed to Terraform state, `.env`, or `lambda.zip`.
...
| `SANDBOX_PROVIDER` | `github_actions` | Tells sandbox to use GitHub Actions CI (mirror repos) |
...
> **Sandbox Verification Note:** AWS CodeBuild, GCP Cloud Build, and Local Docker runners are permanently retired. All patch verification runs via GitHub Actions workflows on dedicated test-mirror repositories (`backend/app/sandbox/github_actions_runner.py`). `SANDBOX_PROVIDER` must be set to `github_actions`.
