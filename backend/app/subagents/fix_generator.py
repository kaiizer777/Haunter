...
def _build_messages(
    ...
    system_prompt = (
        ...
        "\n"
        "If a '## Source Code of Failing Files' section is present, it contains the ACTUAL source "
        "code of the implementation file being tested. Your unified diff MUST modify the exact "
        "functions, variables, and lines from that source code. Do NOT invent new function names or "
        "helper modules when the existing code already has the relevant functions.\n"
        "\n"
        "COMPREHENSIVE FIX: If multiple tests fail or multiple bugs are identified in the diagnosis "
        "or test logs, your patch MUST fix ALL of them across the file(s). Do not fix only one issue "
        "and leave other known failures unaddressed.\n"
        "CRITICAL RULE: Modify the IMPLEMENTATION file(s) (e.g. under app/, src/, backend/app/) to "
        "satisfy the tests. Do NOT modify or delete test assertions to make tests pass.\n"
        "\n"
    ...
    if prior_attempt is not None:
        ...
        prior_content = (
            ...
            f"{strategy_section}"
            "\n"
            f"IMPORTANT: Your new patch will be applied to the ORIGINAL base repository code, "
            f"NOT on top of Attempt #{prior_attempt.attempt_number}. If the prior patch fixed part "
            "of the problem correctly, you MUST include those changes in this new patch as well, "
            "while fixing the remaining test failures. Do NOT repeat the exact same failing patch."
        )
