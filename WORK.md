...
- [x] Add Lambda hosting adapter for orchestrator: FastAPI via `Mangum` + Lambda Function URL (no API Gateway needed — $0) — same webhook `WORK.md:70` + `HAUNTER.md:124` 10s 2xx contract as Cloud Run `HAUNTER.md:147`; scales to zero, no instance to keep alive
- [x] Unified provider switch: `SANDBOX_PROVIDER=gcp|aws` and `HOSTING_PROVIDER=gcp|aws` (env) with optional DB override in `system_configs` table; validated via Pydantic allowlist (`gcp|aws` only), hot-switchable without redeploy (60s TTL cache — same pattern as `LLMClient` `WORK.md:91`)
- [x] Infra as code for AWS: Lambda (ARM64/512MB/900s) + Lambda Function URL, IAM roles, CloudWatch log group via Terraform (`infra/aws/lambda.tf`); GCP infra remains as optional restore target
- [x] Long-term viability: Lambda is **always-free** (1M req + 400k GB-s/mo, not 12mo like EC2 `HAUNTER.md:177`), CodeBuild 100 min/mo always-free — at 1-3 repos / max 10 users ~60 min/week, cost stays $0-$1/mo even after free-tier year, so AWS path is permanent, not 1-2mo bridge; $1 budget alert documented in `lambda_handler.py` and `lambda.tf`
- [x] **[SECURE]** Lambda role least-privilege: `AWSLambdaBasicExecutionRole` + `lambda:InvokeFunction` on self only + `codebuild:StartBuild/BatchGetBuilds` scoped to sandbox project; explicit `Deny` on `secretsmanager:*`, `iam:*`, `ec2:*`; no cross-tenant access — verified via inline IAM policy in `infra/aws/lambda.tf`
- [x] **[SECURE]** Provider switch endpoint/config write is admin-only (same gate as `PUT /config/model` `WORK.md:97`); `SANDBOX_PROVIDER`/`HOSTING_PROVIDER` values allowlisted (`AllowedHostingProvider = Literal["gcp", "aws"]`), never free-text, never derived from request headers
